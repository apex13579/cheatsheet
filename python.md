...
Outer join: A way of combining data such that all the keys from both dataframes get included in the merge
...
- - Python: A high-level, object-oriented programming language designed for readability and efficiency.
- - Library: A reusable collection of code, also referred to as a "package," that allows you to perform complex tasks without writing code from scratch.
- - Dynamic Typing: A feature where variables are not bound to a specific data type; they can point to objects of any type throughout the execution.
- - Naming Conventions: Consistent guidelines used to describe the content, version, or purpose of a file or variable to keep code organized.
- - Variable Algorithm: A mental checklist used when creating a variable:
  - What is the name?
...
- - Vectorization: A process that enables operations to be performed on multiple components of a data object simultaneously, often used in libraries like NumPy.

### Fundamentals & Definitions
# - Loop: A block of code used to carry out iterations, repeating a function or process until a condition is met.
# - Argument: The specific information or descriptions you give a function inside its parentheses to influence its behavior.
# - Object: A fundamental building block of Python; it is an instance of a class.
# - Class: An object’s data type that bundles data and functionality together.
# - Method: A function that belongs to a specific class and typically performs an action or operation on an object.
# - Attributes: Values associated with an object or class, referenced by name using dot notation.
# - Dot Notation: The syntax used to access the methods and attributes that belong to an instance of a class (e.g., `object.method()`).
# - Data Type: An attribute that describes a piece of data based on its values, its language, or the operations it can perform.
# - Assignment: The process of storing a value in a variable's reserved location in memory.
# - Expression: A combination of numbers, symbols, or variables that produce a result when evaluated by the computer.
# - String: A sequence of characters and punctuation that contains textual information; it is an immutable data type.
# - Immutable Data Type: A data type in which the values can never be altered or updated once they are created.
# - Integer: A data type used to represent whole numbers without fractions or decimals.
# - Float: A data type used to represent numbers that contain decimals.
# - Implicit Conversion: The process Python uses to automatically convert one data type to another without user intervention.
# - Explicit Conversion: When a user manually converts the data type of an object to a specific required type (also known as casting).

### Python Syntax & Tools
* - Jupyter Notebook: An open-source web application partitioned into modular "Cells" for creating and sharing live code, formulas, and visualizations.
  - In Jupyter, you can find available methods for an object by typing a `.` and then pressing the `Tab` key.
* - Markdown: A markup language used within coding environments to write formatted text for documentation.
* - print(): A function that returns the input provided to it; `print(example)` will display the value stored in the variable `example`.
* - type(): A function used to identify and return the data type of a specific object or variable.
* - sorted(): A command that returns the inputted argument numbers in order from least to greatest.
* - Assignment vs. Comparison:
  - The single `=` is strictly reserved for assigning values to variables.
  - The double `==` (Comparator) is used in mathematical formulas or logic to check for equality.
* - Mathematical Operators:
  - `**` is used for exponents.
  - `*` is used for multiplication.
  - `%` (Modulo) returns the remainder after division.
  - Python strictly follows the standard order of operations when using `( )`.
* - Boolean Logic: Mathematical comparisons will return `True` if correct and `False` if incorrect.
* - Naming Restrictions: 
  - Variables can only include letters, numbers, and underscores.
  - Variables must start with a letter or an underscore.
  - If a name changes color in your editor, it is a reserved Keyword; do not use it for a variable name.
* - Control Flow (If/Then): Operates on branching logic to alter execution sequences:
  - if age >= 18:
  -     print('adult')
  - else:
  -     print('minor')

### Core Classes & Data Science Vocabulary
* - Core Python Classes: Integers, Floats, Strings, Booleans, Lists, Dictionaries, Tuples, Sets, Frozensets, Functions, Ranges, and None.
# - DataFrame: A two-dimensional, labeled data structure with rows and columns (Pandas).
# - Series: A one-dimensional, labeled array where the data type must be consistent (Pandas).
* - agg(): A pandas groupby method that allows the user to apply multiple calculations to groups of data.
- - Boolean masking: A filtering technique that selects values in a dataframe that align with "True" values on a grid.
* - concat(): A function used to combine data either horizontally (new columns) or vertically (new rows).
* - iloc[] / loc[]: 
  - iloc[] selects data by integer-location-based position.
  - loc[] selects data by the name of the rows or columns.
* - merge(): A function that joins two dataframes together by extending horizontally along axis one.
* - NumPy Attributes:
